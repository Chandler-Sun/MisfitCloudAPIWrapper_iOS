...
#define SET_HEADER(k, v) [r setValue:v forHTTPHeaderField:k];
    ...
    switch (self.method) {
        ...
        case FSNRequestMethodPOST: {
            ...
            if (([_headers.allKeys containsObject:@"Content-Type"])
                && ([@"application/json" isEqualToString:_headers[@"Content-Type"]] || [@"application/x-amz-json-1.0" isEqualToString:_headers[@"Content-Type"]])) {
                NSError *parseJSONError;
                contentType = _headers[@"Content-Type"];
                body = [NSJSONSerialization dataWithJSONObject:self.parameters options:0 error:&parseJSONError];
            }
            else {
    ...
    SET_HEADER(@"Accept-Language", nil);
    SET_HEADER(@"Accept", nil);
    return r;
}
